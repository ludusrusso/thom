...
`thomctl` takes markdown bodies in three ways. **Use them in this order:**

1. **Heredoc into stdin via `-f -`** — the default for PRDs and sub-issues.
   No temp file on disk, no shell-escaping pain, the markdown reads inline:

   ```sh
   cat <<'EOF' | thomctl prd create --title "Improve onboarding" -f -
   ## Goal
   ...
   EOF
   ```

2. **Inline `--body "..."`** — only for short, single-paragraph content
   (e.g. a one-line triage comment). Don't use it for PRD or sub-issue
   bodies; multi-line markdown inside a double-quoted string gets ugly fast.

3. **File path `-f prd.md`** — only if the file already exists for another
   reason. Don't write a temp file just to feed it to `thomctl`; use the
   heredoc form instead.

`-f` is the short alias for `--body-file` (or `--comment-file` on `issue
close`). It accepts a path or `-` for stdin.
...
Make the first-run flow obvious.
...
```sh
cat <<'EOF' | thomctl issue create --parent OPE-742 --title "Wire schema" -f - --afk
## Slice

Wire the Postgres schema and run the migration.
...
cat <<'EOF' | thomctl issue create --parent OPE-742 --title "Pick storage" -f - --hitl
## Decision needed

Postgres vs. SQLite for the local cache.
EOF
```
...
Inline `--body` is fine here — comments are usually short:
...
For a longer comment, use heredoc-stdin:

```sh
cat <<'EOF' | thomctl issue comment OPE-743 -f -
...
EOF
```
