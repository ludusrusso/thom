...
```sh
# Create a PRD inline via heredoc-stdin. Auto-applies PRD + ready-for-agent + engine labels,
# auto-assigns the current user. `-f -` reads markdown from stdin; `-f path.md` reads a file.
cat <<'EOF' | thomctl prd create --title "RC.report" -f -
## Goal
...
EOF
# → PROJ-742
...
# Same `-f -` heredoc-stdin pattern as `prd create`.
cat <<'EOF' | thomctl issue create --parent PROJ-742 --title "Wire schema" -f - --afk
## Slice
...
EOF
thomctl issue create --parent PROJ-742 --title "Pick storage" -f slice2.md --hitl
